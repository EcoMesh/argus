...
class RethinkBackend(BaseBackend):
    ...
    def cleanup(self):
        ...
        result = (
            self.table.filter(r.row["date_done"].lt(self.app.now() - self.expires))
            .delete()
            .run(self.conn)
        )
        if result["errors"]:
    ...
    def __reduce__(self, args=(), kwargs={}):
        kwargs.update(dict(expires=self.expires))
    ...
    def database(self):
        ...
        if self.database_name not in r.db_list().run(self.conn):
    ...
    def table(self):
        ...
        # Ensure table exists
        if self.table_name not in self.database.table_list().run(self.conn):
            self.database.table_create(self.table_name).run(self.conn)

        # Ensure an index on date_done is there, if not process the index
        # in the background. Once completed cleanup will be much faster
        if "date_done" not in table.index_list().run(self.conn):
