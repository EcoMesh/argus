...
import { Room, Check, Water, Layers, Opacity, WaterDrop, ExpandMore, Thermostat } from '@mui/icons-material';
import { List, ListItem, Accordion, Typography, ListItemIcon, ListItemText, ListItemButton, AccordionDetails, AccordionSummary } from '@mui/material';
...
export function Marker({index, active, setActive, sensor, ...props}) {
  ...
  const markup = renderToStaticMarkup(
    <Room style={{
      marginLeft: -16, marginTop: -10, width: 41, height: 41, color,
  ...
  return <MapMarker {...props} icon={icon} eventHandlers={{
    popupclose() {
      setActive(null)
    },
    click() {
    ...
    <Popup>
      <Typography variant='subtitle1'><Link to={`/sensors#${sensor.nodeId}`}>{sensor.nodeId}</Link> <SensorStatusLabel style={{marginLeft: 4}} sensor={sensor} /></Typography>
      <Typography variant='body2' style={{marginTop: 4, marginBottom: 0}}>Latitude: {sensor.location.coordinates[0]}</Typography>
      <Typography variant='body2' style={{margin: 0}}>Longitude: {sensor.location.coordinates[1]}</Typography>
    </Popup>
  </MapMarker>
}
...
const MapLayerItem = ({children, icon, layer, mode, setMode}) => {
  ...
  return (
    <ListItem
      sx={{ p: 0,
        borderTop: `1px solid rgba(${selectedBorder ? '0, 0, 255' : '0, 0, 0'}, .1)`,
      }}
    >
      ...
      <Accordion sx={{ overflow: 'hidden', boxShadow: 5 }} defaultExpanded>
        ...
        <AccordionDetails sx={{ p: 0 }}>
          <List sx={{ p: 0, borderTop: '1px solid rgba(0, 0, 0, .2)' }}>
            <MapLayerItem icon={<Water color='primary'/>} layer={MapMode.WaterLevel} mode={mode} setMode={setMode}>Water Level</MapLayerItem>
            <MapLayerItem icon={<WaterDrop color='info'/>} layer={MapMode.Humidity} mode={mode} setMode={setMode}>Humidity</MapLayerItem>
            <MapLayerItem icon={<Opacity color='primary'/>} layer={MapMode.Moisture} mode={mode} setMode={setMode}>Moisture</MapLayerItem>
            <MapLayerItem icon={<Thermostat color='error'/>} layer={MapMode.Temperature} mode={mode} setMode={setMode}>Temperature</MapLayerItem>
          </List>
        </AccordionDetails>
      </Accordion>
      </div>
